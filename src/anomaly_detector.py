    ...
    def run_pipeline(
        ...
        if run_lstm:
            try:
                ...
                self.lstm_model.save(self.config.get_output_path("models", "lstm_autoencoder.h5"))
            except ImportError as exc:
                notes.append(str(exc))

        if_results = self.if_model.fit_and_detect(prepared["X_train"], prepared["X_test"])
        self.if_model.save(self.config.get_output_path("models", "isolation_forest.pkl"))
        ...
        results_path = self.config.get_output_path("metrics", "anomaly_results.csv")
        ...
        metrics_path = self.config.get_output_path("metrics", "model_metrics.csv")
        ...
        if make_plots:
            ...
            preferred_filename = (
                "lstm_anomaly_detection.png" if lstm_results is not None else "if_anomaly_detection.png"
            )
            ...
            self.visualizer.plot_metric_summary(metrics_df)
        ...
        summary_path = self.config.get_output_path("metrics", "summary.txt")
